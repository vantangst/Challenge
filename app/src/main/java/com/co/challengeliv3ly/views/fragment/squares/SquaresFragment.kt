package com.co.challengeliv3ly.views.fragment.squares
...
@ActionBarOptions(
    title = R.string.title_tab_squares,
    left = false
)
@LayoutId(R.layout.fragment_squares)
class SquaresFragment : AppFragment<MainViewModel>() {

    private val adapter by lazy { ShapeAdapter(requireContext(), rlMainView) }
    private val type = ShapeType.SQUARE.value
    ...
    private fun updateShapeColor(index: Int) {
        ...
        if (DriverUtils.isNetworkEnabled(requireContext())) {
            viewModel.getShapeColor.value = type
        } else {
